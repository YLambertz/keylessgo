...
String NdefRecord::getId()
{
	...
	String strID(id); //eventuell erst in String?
	
	Serial.print(F("    String aus getID() lauet:  ")); Serial.println(strID);
    return id;
}
...
//NEU Yannik Lambertz fuer Projekt KeylessGo 
String NdefRecord::getPayload( )                                   
{
    char payload[_payloadLength + 1];                            //KLAPPT siehe print() :)    
    memcpy(payload, _payload, _payloadLength);					//payload char schreiben mit aktueller _payload //Problem: aktuelle Payload wurde nicht übergeben
    payload[_payloadLength] = '\0' ; 
	//Serial.print(F("    _payload in getpayload  "));PrintHexChar(_payload, _payloadLength);

	String str(payload); //erst in String
	
	Serial.print(F("    String gesendet von getpayload() lauet:    ")); Serial.println(str); //KLAPPT 
...
//NEU
    ...
    memcpy(payload, _payload, _payloadLength);					//payload char schreiben mit aktueller _payload
    payload[_payloadLength] = '\0' ; 
	String str(payload); //eventuell erst in String?
	Serial.print(F("    Inhalt von _payload aus NdefRecord.print: ")); Serial.println(str); //eventuell so ? 
